...
public class ImageToDatabase extends Application
{
    ...
    public void start(Stage primaryStage)
    {
        ...
        //creating button to show the uploaded photo
        Button retrieve = new javafx.scene.control.Button("retrieve");
        //creating button to help us upload our photo
        Button uploadB = new Button("Upload");

        bp.setTop(retrieve);//placing the button at the bottom
        ...
        retrieve.setOnAction((ActionEvent t) ->{
            try {
                conn = java.sql.DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","Yabs2030");
                PreparedStatement ps = conn.prepareStatement("SELECT * FROM test.photos where id = 1;");
                ResultSet resultSet = ps.executeQuery();

                while (resultSet.next()){
                    byte[] imagefile = resultSet.getBytes(2);
                    Image image = new javafx.scene.image.Image(String.valueOf(imagefile));
        ...
        uploadB.setOnAction((ActionEvent t) -> {
            ...
            try {
                ...
                conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","Yabs2030");
